...
slider.noUiSlider.on("update", function (values, handle) {
  ...
  $(".VBF").css(
    "width",
    Math.round(25 + Math.round((values[handle] / 13500) * 100) / 4) + "%"
  );
  ...
  if ($(window).width() > 1023) {
    ...
    $("#VBP").css(
      "height",
      (25 + Math.round((values[handle] / 13500) * 100) / 4) / 2 + 1.25 + "vh"
    );
  }
});
...
function SKIN(Dx) {
  ...
  if ($(Dx).hasClass("opacity-100") == true) {
    ...
    $("img", Dx)
      .removeClass("FORTNITE-YELLOW-BORDER")
    ...
    $("img", Dx)
      ...
      .addClass("FORTNITE-YELLOW-BORDER");
  }
...
function FLICK() {
  ...
  if (FUSER == "" || !PL) {
    ...
    SSUSER =
      '<span id="USERUSERUSER">' +
      FUSER +
      '</span><small class="opacity-80 ml-1" id="PLPLPL">(' +
      PL +
      ")</small>";
